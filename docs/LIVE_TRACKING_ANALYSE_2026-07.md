...
>
> **Méthode** : lecture statique du code (pas de toolchain ici — chaque proposition devra être validée au build Jetson). Les références `fichier:ligne` correspondent au commit **`85c245c`** (état AVANT les lots — la branche a bougé depuis, les numéros de ligne ne matchent plus le HEAD).
>
> **Documents liés** : [LIVE_TRACKING_PLAN.md](LIVE_TRACKING_PLAN.md) (plan Phases 1–3, implémenté en PR #20) · [JETSON_SESSION_LOG.md](JETSON_SESSION_LOG.md) (suites 94–101 : flicker, dé-throttle flow, re-anchor, diagnostic overlay ~1 s) · [JETSON_ERREURS.md](JETSON_ERREURS.md) (#35 masque board, #47 jitter, **#51/#52 ouverts par cette analyse, résolus en suite 109**)
...
- `m_staticThreshPx` (0,8) et `m_flowRedetectInterval` (30 → **120** depuis la suite 112) sont codés en dur — à exposer en config avancée seulement si le tuning terrain le demande.
